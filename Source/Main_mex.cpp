...
int main ( int argc, char** argv ) {
   ...
   //=========================================================
	...
	strcpy(myargv[9], "-cM"); strcpy(myargv[10], "1");
	strcpy(myargv[11], "-nM"); strcpy(myargv[12], "3");
	strcpy(myargv[13],"-D");
...
#pragma omp parallel for
	...
	for (int run = 0; run < num_out_cols; run++ ) {
        ...
        printf("[INFO] Run complexity measures %d / %d ... \n", run, num_out_cols);
        /*
        for(int rr=0; rr < 100; rr++) {
			for(int cc=0; cc < ncols-1; cc++)
				printf("%0.2f ", n_data[rr*(ncols-1) + cc]);
			printf("\n");
		}
		for(int rr=0; rr < nrows; rr++)
			printf("%d ", class_of_sample[rr]);
		printf("\n");
		for(int cc=0; cc < ncols; cc++)
			printf("%d %0.2f %0.2f\n", col_type[cc], att_min[cc], att_max[cc]);
		if(dmat == NULL)
			printf("NULL\n");
        */
        for(int rr=0; rr < nrows; rr++)
	    ...
	    dSet = new ComplexityMeasures ( n_data, class_of_sample, nrows, ncols, col_type, att_min, att_max, dmat,
                                     opts.getTypeOfContinuousDistFunction (), opts.getTypeOfNominalDistFunction (), rmNA);
