...
const int ComplexityMeasures::PROPORTION_EXAMPLES_CONVEX_HULL = 2;
const float ComplexityMeasures::EPSILON_SPHERES = 0.55;
const float ComplexityMeasures::C = 0.05;
const float ComplexityMeasures::TOLERANCE = 0.001;
const float ComplexityMeasures::EPSILON = 0.001;
...
void ComplexityMeasures::getNearestNeighborIntraAndInterClass ( int iExample, int& neigIntra, float& minDistIntra,
    ...
    for ( i = 0; i < numberOfExamples; i++ ) {
        if ( i != iExample ) {
            ...
            distAux = getDistanceFromCache(iExample, i);
...
int** ComplexityMeasures::computePrim () {
    ...
    for ( i = 1; i < numberOfExamples; i++ ) {
        ...
        edge[i] = getApproximateDistanceFromCache(currentNode, i);
    }
    ...
    for ( i = 1; i < numberOfExamples; i++ ) {
        ...
        for ( j = 0; j < numberOfExamples; j++ ) {
            ...
            */
            float dis = getApproximateDistanceFromCache(currentNode, j);
            if ( neig[j] != -1 && edge[j] > dis ) {
                neig[j] = currentNode;
                edge[j] = dis;
            }
        }
...
float ComplexityMeasures::computeNonLinearityKNNTrain ( int k ) {
    ...
    return  runKNN ( k, example, classOfExample, numberOfExamples, true, true );
...
float ComplexityMeasures::computeNonLinearityKNNConvexHull ( int k ) {
    ...
    result = runKNN ( k, testExamples, classOfTestExamples, numInstToGeneratePerClass * numberOfClasses, false, false );
...
float ComplexityMeasures::runKNN ( int k, float** testExamples, int* classOfTestExamples,
                                  int numberOfTestExamples, bool isTrain, bool isOriData ) {
    ...
    for ( i = 0; i < numberOfTestExamples; i++ ) {
        ...
        for ( j = 0; j < numberOfExamples; j++ ) {
            ...
            if ( !isTrain || i != j ) {
                ...
                if(isOriData) //N3
                    dNode.dist = getApproximateDistanceFromCache ( i, j );
                else {
                    //dNode.dist = getApproximateDistance ( testExamples[i], example[j] );
                    float dis = 0;
                    for(int att=0; att < numberOfAttributes; att++)
                        dis += (testExamples[i][att] - example[j][att])*(testExamples[i][att] - example[j][att]);
                    dNode.dist = dis;
                }
            ...
            if ( oClass != cClass ) {
                for ( i = 0; i < numberOfExamplesPerClass[ cClass ]; i++ ) {
                    ...
                    for ( j = 0; j < numberOfExamplesPerClass[ oClass ]; j++ ) {
                        ...
                        dist = getDistanceFromCache ( indexExamplesPerClass[ cClass ][i], indexExamplesPerClass[oClass][j] );
            ...
            for ( i = 0; i < numberOfExamplesPerClass[ cClass ]; i++ ) {
                ...
                if ( adherenceOrder[ cClass ][i] == maximum ) { // Example really far from the boundary.
                    ...
                    for ( j = 0; j < numberOfExamplesPerClass[ cClass ]; j++ ) {
                        ...
                        dist = getDistanceFromCache ( indexExamplesPerClass[ cClass ][i], indexExamplesPerClass[ cClass ][j] );
...
float ComplexityMeasures::computeFisherVectorized () {
    ...
    try  {
        ...
        if ( isnan ( fisherValue ) || isnan ( denom.getValue ( 0, 0 ) ) ) {
            return 0;
        }
        else {
