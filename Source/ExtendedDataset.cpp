...
ExtendedDataset::ExtendedDataset ( std::string fileName, bool readAttInfo, bool repUnknownVal,
    ...
    calDistanceMatrix();
    std::cout << "Cal distance matrix time: " << Utils::getTime() - start_t << std::endl;
...
ExtendedDataset::ExtendedDataset (  const float* n_data, const int* class_values, const int rows, const int cols, 
                                    const int* att_type,
                                    const double* att_min, const double* att_max, float* dmat,
                                    int realAttributesFunction, int nominalAttributesFunction, bool rmNA, bool ori_version) :
                                    Dataset(n_data, class_values, rows, cols, att_type, att_min, att_max, dmat, rmNA, ori_version ) {
    // Initialize the class variables.
    numberOfExamplesPerClass    = 0;
    examplesPerClass       = 0;
    indexExamplesPerClass  = 0;
...
void ExtendedDataset::calDistanceMatrix() {
    dMat = new float[numberOfExamples*numberOfExamples];

    if(dMat == NULL) {
        Utils::logWarningError ( "Cannot allocale mem dMat" );
        throw std::string ( " Cannot allocale mem dMat " );
    }

    for(int i=0; i < numberOfExamples; i++) {
        for(int j=0; j < numberOfExamples; j++) {
            if(i == j)
                dMat[i*numberOfExamples + j] = 0;
            else if (i < j) {
                float dist = 0;
                for(int att = 0; att < numberOfAttributes; att++) {
                    if(example[i][att] == Dataset::UNKNOWN_VALUE || example[j][att] == Dataset::UNKNOWN_VALUE)
                        dist+= 1;
                    else {
                        //float v = distanceFunction[att]->computeDistance(example[i][att], example[j][att]);
                        float v = example[i][att] - example[j][att]; 
                        dist += v*v; 
                    }
                }
                dist = sqrt(dist);
                dMat[i*numberOfExamples + j] = dist;
                dMat[j*numberOfExamples + i] = dist;
            }
        }
    }
}
...
float ExtendedDataset::getDistanceFromCache(int i, int j) {
    return dMat[i*numberOfExamples+j];
}


float ExtendedDataset::getApproximateDistanceFromCache(int i, int j) {
    return dMat[i*numberOfExamples+j]*dMat[i*numberOfExamples+j];
}
