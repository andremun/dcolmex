...
Dataset::Dataset ( const float* n_data, const int* class_values, const int rows, const int cols, 
                   const int* att_type,
                   const double* att_min, const double* att_max, float* dmat, bool rmNA, bool ori_version ) {
    // Initialize the attributes of the environment.
    initAttributes ();

    // Initialize two variables to count the number of attributes and examples.
    numberOfAttributes = cols-1;
    numberOfExamples   = rows;
    ...
    // class info
    std::set<int> class_labels;
    for(int i=0; i < numberOfExamples; i++)
        class_labels.insert(class_values[i]);
    
    classPosition = cols - 1;
    classType = 'N';
    numberOfClasses = class_labels.size();
    minClassValue = 0;
    maxClassValue = numberOfClasses - 1;

    nominalClassValues = new std::string[ numberOfClasses ];
    for(int i = 0; i < numberOfClasses; i++) {
        int c = *std::next(class_labels.begin(), i);
        std::ostringstream ss;
        ss << c;
        nominalClassValues[i] = ss.str();
#ifdef PRINT_DEBUG
        std::cout << "CLASS: " << ss.str() << std::endl;
#endif
    }

    for(int row = 0; row < rows; row++) {
        int c = class_values[row];
        int ind = 0;
        for (int i=0; i < numberOfClasses; i++) {
            if(std::stoi(nominalClassValues[i]) == c) {
                classOfExample[row] = ind;
                break;
            }
            ind++;
        }
    ...
    // attributes info
    for(int attCount = 0; attCount < numberOfAttributes; attCount++) {

        if(att_type[attCount] == NUMERICAL) {
            typeOfRepresentation[ attCount ] = 'R';
            numberOfContinuousAttributes++;
            if(ori_version) {
                minAttValue[ attCount ] = (float)att_min[attCount];
                maxAttValue[ attCount ] = (float)att_max[attCount];
            }
            else {
                minAttValue[ attCount ] = 0.0;
                maxAttValue[ attCount ] = 1.0;
            }
            ...
            minAttValue[ attCount ] = 0.0;
            maxAttValue[ attCount ] = att_max[attCount];
            nominalAttrNumValues[ attCount ] = att_max[attCount] + 1;

            // Reserve memory to store all the nominal identifiers.
            enumAtt[ attCount ] = new std::string [ ( int ) maxAttValue[ attCount ] + 1 ];

            int j = 0;
            for(int i=0; i < att_max[attCount] + 1; i++) {
                ostringstream ss;
                ss << i;
                enumAtt[ attCount ][j++] = ss.str();
            }
        }
    }
    ...
    //readData ( fin, readAttInfo );
    for(int row = 0; row < rows; row++) {
        for (int col=0; col < numberOfAttributes; col++) {

            if(n_data[row*numberOfAttributes + col] == Dataset::UNKNOWN_VALUE) {
                example[row][col] = Dataset::UNKNOWN_VALUE;
                continue;
            }

            example[row][col] = n_data[row*numberOfAttributes + col];
        }
    }
        ...
        for ( att = 0; att < numberOfAttributes; att ++ ) {
            if ( example[i][ att ] == Dataset::UNKNOWN_VALUE ) {
                ...
                if ( typeOfRepresentation[ att ] == 'R' || typeOfRepresentation[ att ] == 'I' ) {
                    ...
                    //example[i][ att ] = Utils::roundf ( avgRealValue[ att ][ classOfExample[i] ] );
                    example[i][ att ] = avgRealValue[ att ][ classOfExample[i] ];
                }
            }
        }
