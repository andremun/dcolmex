...
//// COMPARISONS
    ...
    static void logWarningError ( std::string logMessage ) {
    	...
    	if ( fLog ) {
    	    fLog << logMessage.c_str() << std::endl;
    	}
    ...
    static void distanceCPU(const float*vg_a, int n_a, int k, float* d, float UNKNOWN_VALUE);
    static void distanceApproximateCPU(float** vg_a, float** vg_b, int n_a, int n_b, int k, float* d);

    static float* loadDataFromArffFile(const char* filename, int& nrows, int& ncols, vector<Attribute>& att_type, 
                                        bool& has_na, float UNKNOWN_VALUE = 100000.);

    static int removeNA(const float* data, const int nrows, const int ncols, const bool*na_row_mask, float* data2);
