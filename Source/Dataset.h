...
//// ATTRIBUTES TO STORE THE AVERAGE AND THE STANDARD DEVIATION OF EACH ATTRIBUTE PER CLASS
    ...
    /** TN - distance matrix to speed up getDistance func */
    float* dMat;
    bool dMatAvai;
...
public:
    ...
    //mex - for both bug-fixed and original versions
    Dataset ( const float* n_data, const int* class_values, const int rows, const int cols, 
              const int* att_type,
              const double* att_min, const double* att_max, float* dmat, bool rmNA = true, bool ori_version = false );
